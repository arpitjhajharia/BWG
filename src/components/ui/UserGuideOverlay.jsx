...
const fallbackGuide = {
    title: 'User Guide',
    subtitle: 'No specific guide available for this module yet.',
    sections: [
        {
            icon: 'ℹ️',
            ...
            items: [
                ...
                'Most tables support search, sort, and filter functionality.',
                'Click the ⓘ icon on any tab to check for a guide.',
            ],
        },
    ],
};

export const UserGuideOverlay = ({ activeTab, onClose }) => {
    const guide = guides[activeTab] || fallbackGuide;

    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in" onClick={onClose}>
            <div
                className="bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[85vh] flex flex-col overflow-hidden border border-slate-200"
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-slate-50">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-blue-100 rounded-lg flex items-center justify-center border border-blue-200">
                            <Icons.Info className="w-5 h-5 text-blue-600" />
                        </div>
                        <div>
                            <h2 className="font-black text-base text-slate-800 leading-tight">{guide.title}</h2>
                            <p className="text-[11px] font-medium text-slate-400 mt-0.5">{guide.subtitle}</p>
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    {guide.sections.map((section, i) => (
                        <div key={i}>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <h3 className="text-[12px] font-black uppercase tracking-widest text-slate-700">{section.heading}</h3>
                            </div>
                ...
                <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/60 flex items-center justify-between">
                    <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">Nizona OS • User Guide</span>
                    <button
                        onClick={onClose}
                        className="px-4 py-1.5 bg-blue-600 text-white text-[11px] font-bold uppercase tracking-widest rounded-md hover:bg-blue-700 transition-colors shadow-sm"
                    >
                        Got It
                    </button>
                </div>
